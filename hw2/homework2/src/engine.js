...
async function GAMES202Main() {
	...
	for (let i = 0; i < envmap.length; i++) {
		...
		let val = '';
		await this.loadShaderFile(envmap[i] + currentSHtype +"/transport.txt").then(result => {

			console.log("transport.txt loaded");
			val = result;
		});
		...
		await this.loadShaderFile(envmap[i] +  currentSHtype + "/light.txt").then(result => {
			console.log("light.txt loaded");
			val = result;
		});
