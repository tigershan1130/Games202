        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    // we get the direction of environment maps 
                    ...
                    for (int l = 0; l <= SHOrder; l++)
                    {
                        ...
                        for (int m = -l; m <= l; m++)
                        {
                            // projected angle coefficent
                            float angle = CalcArea(x, y, width, height);
                            double Sh_Coff = sh::EvalSH(l, m, dir.cast<double>());

                            // integral Lenv * angle * sh_Coff
                            SHCoeffiecents[sh::GetIndex(l, m)] += Le * Sh_Coff * angle;
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            // already going through the surface samples
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                else
                {
                    ...
                    {                           
                        Ray3f ray;
                        ray = Ray3f(v, wi);
                        return (!scene->rayIntersect(ray)) ? h : 0.f;
                    }
        ...
        }// end for vertex count
        if (m_Type == Type::Interreflection)
        {
            // we still need go through our vertex count
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                const Point3f &v = mesh->getVertexPositions().col(i);
                const Normal3f &n = mesh->getVertexNormals().col(i);
