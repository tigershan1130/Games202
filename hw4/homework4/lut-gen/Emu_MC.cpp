...
/* n2 is air = 1.0 
 n1 is related to roughness*/
Vec3f fresnelSchlick(Vec3f F0, Vec3f V, Vec3f H)
{
    float HdotV = std::max(dot(V, H), 0.0f);

    return F0 + (Vec3f(1) - F0) * pow(1.0 - HdotV, 5.0);
}
...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      // DOING... not sure where it is going wrong...
        ...
        if (NdotL > 0.0)
        {
            ...
            // glass to air reflection coefficient for R0 is 0.04
            Vec3f F = fresnelSchlick(Vec3f(0.2, 0.2, 0.2), V, H);
            float G = GeometrySmith(roughness, NdotV, NdotL);

            float BRDF = D * G / std::max((4 * NdotV * NdotL), 0.0001f);
            A += BRDF * NdotL / pdf;
        }
    }
    B = A;
